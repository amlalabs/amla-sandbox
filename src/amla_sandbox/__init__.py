...
Quick start (LangGraph)::
    ...
    from amla_sandbox import SandboxTool, create_bash_tool
    from langgraph.prebuilt import create_react_agent

    def get_weather(city: str) -> str:
        return f"Sunny in {city}"

    # Easiest: bash-style tool with auto caps
    bash = create_bash_tool(tools=[get_weather])
    agent = create_react_agent(model, [bash.as_langchain_tool()])

    # Or wire capabilities yourself
    sandbox_tool = SandboxTool.from_functions([get_weather])
...
# Simple bash tool API (AI SDK-style ergonomics)
from .bash_tool import create_bash_tool

__all__ = [
    # === High-level API (recommended) ===
    # Simple bash tool (AI SDK-style ergonomics)
    "create_bash_tool",  # Dead-simple: create_bash_tool(tools=[...])
