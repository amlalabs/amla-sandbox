"""AI SDK-style bash tool for Amla sandbox.

This module provides a dead-simple API for creating bash-enabled tools
...
Quick Start::
    ...
    # Layer 0: Just works - shell only
    bash = create_bash_tool()

    # Layer 1: With tools
    bash = create_bash_tool(tools=[get_weather, send_email])
    ...
    agent = initialize_agent(tools=[bash], ...)

The developer shouldn't need to understand WASM, PIC, or capability tokens
to get started. They just get a shell that's safer than subprocess.
"""
...
def create_bash_tool(
    ...
    *,
    ...
    """Create a bash tool for AI agents.

    This is the simplest way to give an AI agent shell access with optional
    ...
    Examples::
        ...
        bash = create_bash_tool()
        result = bash.run("echo 'hello' | wc -c")

        # With tools - auto-generates capabilities
        bash = create_bash_tool(tools=[get_weather, search_db])
        result = bash.run("tool get_weather --city SF")
        # or: bash.run("await get_weather({city: 'SF'})")
